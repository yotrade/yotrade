...
| Trading on a wide or broken price | A fill that adds risk reverts when the confidence interval exceeds a quarter of the maintenance margin (12.5 bps at 100x). Every fill reverts on non-positive prices or exponents outside `[-18, 0]` |
| Cashing in the oracle's doubt on the way out | A fill that only reduces or closes is never refused for confidence: it fills at the edge of the band that is worse for the trader (a sell at price − conf, a buy at price + conf), so an uncertain price can only cost the trader who leaves |
| Unlimited leverage as a free option, since losses are floored at zero | Fills that add risk, a flip through zero included, must keep total notional within the tournament's cap (100x by default; an organizer may choose 5x or 20x before the start) across all markets. Anyone can liquidate an account under the maintenance margin, half the initial margin (10%, 2.5% or 0.5%), which rivals are motivated to do; a liquidator in CI does it when nobody else does. Pyth's confidence must stay under a quarter of that margin or the fill is refused |
...
| Locking a trader into a position | Fills that reduce risk skip the leverage, market and confidence checks. Pausing never blocks `settle` |
